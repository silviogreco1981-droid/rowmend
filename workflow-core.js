(function (root, factory) {
  ...
  function projectConfigFingerprint(project) {
    const artifacts = project?.artifacts || {};
    return hashText(stableSerialize({
      cleanRecipe: artifacts.cleanRecipe?.recipe || null,
      dataContract: artifacts.dataContract?.contract || null,
      importProfile: artifacts.importProfile?.profile || null
    }));
  }

  function profileMetrics(profile) {
    if (!profile || !profile.summary || !Array.isArray(profile.columns)) return null;
    return {
      rows:Number(profile.summary.rows || 0),
      columns:Number(profile.summary.columns || 0),
      completeness:Number(profile.summary.completeness || 0),
      duplicateRows:Number(profile.summary.duplicateRows || 0),
      mixedColumns:Number(profile.summary.mixedColumns || 0),
      allMissingColumns:Number(profile.summary.allMissingColumns || 0),
      columnMetrics:profile.columns.map(column => ({
        name:String(column.name || ''),
        type:String(column.type || 'string'),
        missingRate:Number(column.missingRate || 0),
        uniqueRate:Number(column.uniqueRate || 0),
        mixedTypeRate:column.nonEmpty ? Number(column.mixedTypeCount || 0) / Number(column.nonEmpty) : 0
      }))
    };
  }
  ...
  function generateSql(dataset, validation, importProfile, mode = 'insert') {
    ...
    if (mode === 'insert') {
      ...
      const header = [
        '-- Generated by RowMend Workflow Runner 0.9.0',
  ...
  function runWorkflow(dataset, project, options = {}) {
    ...
    const finalProfile = dataCore.profileDataset(working.rows, working.headers);
    ...
    if (contract) {
      contractResult = contractCore.checkContract(working, finalProfile, contract);
    ...
    return {
      ...
      finalProfile,
      configFingerprint:projectConfigFingerprint(project),
  ...
  function runSummaryForHistory(result) {
    return {
      ...
      configFingerprint:result.configFingerprint || null,
      profileMetrics:profileMetrics(result.finalProfile || result.originalProfile)
    };
  }
