(() => {
  ...
  function validateRows() {
    ...
    if (duplicateTargetNames.length) {
      issues.push({ level:'error', title:'Duplicate target column names', detail:`Each source column must map to a unique target. Duplicate target: ${duplicateTargetNames.join(', ')}` });
    }
  ...
  function saveProfile(){const name=$('profileName').value.trim();if(!name){alert('Enter a profile name first.');return;}const p=getProfiles();p[name]={tableName:$('tableName').value,dialect:$('dialect').value,keyColumn:$('keyColumn').value,config:state.config};setProfiles(p);refreshProfiles();$('profileSelect').value=name;}
