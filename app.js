(() => {
  ...
  function defaultConfig(schema) {
    ...
    schema.forEach(c => {
      cfg[c.name] = {
        ...
        required: false,
        unique: false,
        email: /email/i.test(c.name),
        type: c.type === 'string' ? 'auto' : c.type
      };
    });
    return cfg;
  }
  ...
  function validateRows() {
    ...
    state.headers.forEach(header => {
      ...
      if (cfg.unique) uniqueSeen[header] = new Map();
    });
    ...
    if (invalidRows) issues.unshift({ level:'error', title:`${invalidRows} row${invalidRows===1?'':'s'} fail configured validation rules`, detail:'Open Mapping & Rules or export the error CSV for row-level details.' });
  ...
  function generateInsert() {
    const d=$('dialect').value, table=qIdent($('tableName').value || 'TARGET_TABLE',d), rows=transformedRows(false).slice(0,1000), cols=mappedColumns();
    ...
    setSql(`-- Generated by RowMend MVP 0.2\n-- ${rows.length}${state.rows.length>rows.length?' of '+state.rows.length:''} rows\n\n${stmts.join('\n')}`, 'INSERT statements');
  }

  function generateMerge() {
    const d=$('dialect').value, keySource=$('keyColumn').value; if(!keySource) return;
    const cols=mappedColumns(), key=cols.find(c=>c.source===keySource)?.target || keySource, nonKey=cols.filter(c=>c.target!==key), table=qIdent($('tableName').value || 'TARGET_TABLE',d), rows=transformedRows(false).slice(0,250);
    const out=[];
    rows.forEach(r=>{
      if(d==='oracle'){
  ...
  function render() {
    ...
    renderMapping(); renderDerived(); refreshProfiles();
  }
