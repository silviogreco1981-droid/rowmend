(() => {
  ...
  function loadRows(rows,name='sample.csv'){
    ...
    if (window.RowMendProjectContext?.getArtifact('importProfile')) {
      loadImportProfileFromProject(true);
    }
  }
  ...
  function applyImportProfile(saved, source = 'local') {
    if (!saved || typeof saved !== 'object') return false;
    if (!state.headers.length) return false;

    $('tableName').value = saved.tableName || $('tableName').value;
    $('dialect').value = saved.dialect || $('dialect').value;
    state.headers.forEach(header => {
      if (saved.config?.[header]) {
        state.config[header] = { ...state.config[header], ...saved.config[header] };
      }
    });
    renderMapping();
    renderDerived(saved.keyColumn);
    if (source === 'project') track('project_artifact_loaded', { artifact:'import_profile' });
    return true;
  }

  function saveImportProfileToProject() {
    const context = window.RowMendProjectContext;
    if (!context?.project) return;
    if (!state.headers.length) {
      alert('Load a file before saving import settings to the project.');
      return;
    }

    try {
      context.saveArtifact('importProfile', {
        profile: currentImportProfile()
      }, { label: $('tableName').value ? `${$('tableName').value} import setup` : 'Project import setup' });
      track('project_artifact_saved', { artifact:'import_profile', columns:state.headers.length });
    } catch (error) {
      alert(error.message || 'Unable to save import settings to the project.');
    }
  }

  function loadImportProfileFromProject(silent = false) {
    const context = window.RowMendProjectContext;
    const artifact = context?.getArtifact('importProfile');
    if (!artifact?.profile) {
      if (!silent) alert('The active project does not contain import settings yet.');
      return false;
    }
    if (!state.headers.length) {
      if (!silent) alert('Load a file before applying project import settings.');
      return false;
    }

    const applied = applyImportProfile(artifact.profile, 'project');
    if (!silent && applied) activateTab('mapping');
    return applied;
  }
  ...
  if (window.RowMendProjectContext?.project) {
    window.RowMendProjectContext.addAction('Save import setup', saveImportProfileToProject);
    window.RowMendProjectContext.addAction('Load project setup', () => loadImportProfileFromProject(false));
  }
